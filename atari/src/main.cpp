...
void setup()
{
  // ps2_init() is intentionally lazy-initialized by ps2_get_hid() from
  // emu_task on core 0. This keeps the PS/2 GPIO ISR away from the
  // I2S video ISR, which runs on core 1.
  mount_filesystem();   // mount the filesystem!
  audio_volume_init();  // restore saved F9/F10 volume; mute always starts off
...
void perf()
{
  static int _next = 300;
  static int prev_drawn = 0;
  static int prev_displayed = 0;

  if (_drawn >= _next)
  {
    int dd = _drawn - prev_drawn;
    int dp = _frame_counter - prev_displayed;
    float elapsed_us = dd * 1000000.0f / (_emu->standard ? 60 : 50);
    float ratio = dd ? (float)dp / (float)dd : 0.0f;

    printf("PERF300 frame_time:%d drawn:%d displayed:%d ddraw:%d ddisp:%d ratio:%1.3f blit_ticks:%d->%d isr:%2.2f%%\n",
           _frame_time / 240, _drawn, _frame_counter, dd, dp, ratio,
           _blit_ticks_min, _blit_ticks_max, (_isr_us * 100.0f) / elapsed_us);

    prev_drawn = _drawn;
    prev_displayed = _frame_counter;
    _next = _drawn + 300;
