...
static esp_err_t start_dma(int line_width, int samples_per_cc, int ch = 1)
{
    ...
    // setup interrupt
    if (esp_intr_alloc(ETS_I2S0_INTR_SOURCE, ESP_INTR_FLAG_LEVEL3 | ESP_INTR_FLAG_IRAM,
...
// Fast PAL active-line prefix: hsync + burst in one call.
void IRAM_ATTR pal_active_prefix(uint16_t *line)
{
    for (int i = 0; i < _hsync; ++i)
        line[i] = SYNC_LEVEL;
    int16_t *b = ((_line_counter - 1) & 1) ? _burst0 : _burst1;
    uint16_t *d = line + _burst_start;
...
{
    const int syncwidth = vbl ? _hsync_long : _hsync;

    // DMA buffers and all sync widths are 32-bit aligned. Store two equal
    // 16-bit DAC samples per write; blanking used to be the only ISR path
    // which could take longer than one 64 us PAL scanline.
    const uint32_t sync_sample = (uint16_t)SYNC_LEVEL;
    const uint32_t blank_sample = (uint16_t)BLANKING_LEVEL;
    const uint32_t sync_pair = sync_sample | (sync_sample << 16);
    const uint32_t blank_pair = blank_sample | (blank_sample << 16);
    uint32_t *dst = (uint32_t *)line;
    const int sync_words = syncwidth >> 1;
    const int line_words = _line_width >> 1;
...
uint32_t _audio_r = 0;
uint32_t _audio_w = 0;
static int _audio_volume = 5; // 0..10; start at 50 percent
static bool _audio_muted = false;

#ifdef ESP_PLATFORM
static bool _audio_volume_dirty = false;
static uint32_t _audio_volume_changed_ms = 0;

void audio_volume_init()
{
    Preferences prefs;
    if (prefs.begin("esp8bit", false)) {
        int volume = prefs.getUChar("volume",5);
        _audio_volume = volume >= 0 && volume <= 10 ? volume : 5;
        prefs.end();
        printf("AUDIO volume restored: %d%%\n",_audio_volume*10);
    } else {
        printf("AUDIO Preferences begin failed while loading volume\n");
    }
    _audio_muted = false;
    _audio_volume_dirty = false;
}

static void audio_volume_changed()
{
    _audio_volume_dirty = true;
    _audio_volume_changed_ms = millis();
}

void audio_volume_save_now()
{
    if (!_audio_volume_dirty)
        return;

    Preferences prefs;
    if (prefs.begin("esp8bit", false)) {
        size_t written = prefs.putUChar("volume",(uint8_t)_audio_volume);
        if (written == sizeof(uint8_t)) {
            _audio_volume_dirty = false;
            printf("AUDIO volume saved: %d%%\n",_audio_volume*10);
        } else {
            printf("AUDIO volume save failed: wrote %u bytes\n",(unsigned)written);
        }
        prefs.end();
    } else {
        printf("AUDIO Preferences begin failed while saving volume\n");
    }
    if (_audio_volume_dirty)
        _audio_volume_changed_ms = millis();
}

void audio_volume_save_update()
{
    if (_audio_volume_dirty &&
        (uint32_t)(millis()-_audio_volume_changed_ms) >= 2000)
        audio_volume_save_now();
}
#else
void audio_volume_init() {}
void audio_volume_save_now() {}
...
int audio_volume_adjust(int delta)
{
    int old_volume = _audio_volume;
    int volume = _audio_volume + delta;
    if (volume < 0)
        volume = 0;
    if (volume > 10)
        volume = 10;
...
int audio_mute_toggle()
{
    _audio_muted = !_audio_muted;
    return _audio_muted ? -1 : _audio_volume * 10;
}
...
void audio_write_16(const int16_t *s, int len, int channels)
{
    ...
    while (len--)
    {
        ...
        if (b > 31)
            b = 31;
        b = _audio_muted ? 0 : (b * _audio_volume) / 10;
