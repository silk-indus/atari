...
public:
    ...
    void page_v(int dir)
    {
        ...
        if (c <= 0)
            return;
        int page = max(1,_overlay->OVERLAY_HEIGHT-2);
        _hilited += dir * page;
        _hilited = max(0,min(_hilited,c-1));
    }
    ...
    void repeat_keys()
    {
        ...
        switch (_repeat_key) {
            case 82: move_v(-1); break;   // up
            case 81: move_v(1);  break;   // down
            case 75: page_v(-1); break;   // page up
            case 78: page_v(1);  break;   // page down
            case 80: move_tab(-1); break; // left
            case 79: move_tab(1);  break; // right
        }
        _click = 1;
        _repeat_count = 4;
    }
    ...
    bool key(int keycode, int pressed, int mods)
    {
        ...
        // F8: global mute toggle, available both in GUI and emulator.
        if (keycode == 65) {
            if (pressed) {
                int volume = audio_mute_toggle();
                if (volume < 0) {
                    msg("Muted");
                } else {
                    char volume_msg[32];
                    snprintf(volume_msg,sizeof(volume_msg),"Volume: %d%%",volume);
                    msg(volume_msg);
                }
            }
            return true;
        ...
        // F9/F10: global volume down/up, available both in GUI and emulator.
        // Consume key-up too so these shortcuts never reach the emulated machine.
        if (keycode == 66 || keycode == 67) {
            if (pressed) {
                int volume = audio_volume_adjust(keycode == 66 ? -1 : 1);
                char volume_msg[32];
                snprintf(volume_msg,sizeof(volume_msg),"Volume: %d%%",volume);
                msg(volume_msg);
            }
        ...
        // F12: hard reset ESP32 from GUI or emulator.
        if (pressed && keycode == 69) {
            audio_volume_save_now();
            esp_restart();
            return true;
        }

        if (_visible && (keycode == 75 || keycode == 78 ||
                         keycode == 79 || keycode == 80 ||
                         keycode == 81 || keycode == 82)) {
            if (pressed) {
                _repeat_key = keycode;
                _repeat_count = 18; // initial delay, about 0.35 s at ~50 updates/s
            } else if (_repeat_key == keycode) {
                _repeat_key = 0;
            }
        }
...
static uint8_t _last_keys[6] = {0};
static int _last_mods = 0;


// Je HID key v 6-key reporte?
static bool hid_key_present(const uint8_t* keys, uint8_t key)
{
    if (!key)
        return false;
...
static void keyboard(const uint8_t* d, int len)
{
    ...
    for (int i = 0; i < 6; i++) {
        ...
        uint8_t key = _last_keys[i];

        if (key && !hid_key_present(keys, key)) {
            gui_key(key, 0, mods);
        }
    }
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (key && !hid_key_present(_last_keys, key)) {
            gui_key(key, 1, mods);
        }
    }


    // Save complete keyboard state
    memcpy(_last_keys, keys, 6);
    _last_mods = mods;
}
