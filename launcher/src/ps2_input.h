...
static volatile uint32_t ps2_last_edge_ccount = 0;
...
static inline uint32_t IRAM_ATTR ps2_cycle_count()
{
    uint32_t count;
...
{
    uint32_t now = ps2_cycle_count();
    // A missing edge during flash access leaves the decoder in the middle of
    // a frame. A valid PS/2 bit stream never has a 0.5 ms intra-frame gap.
    if (ps2_bit && (uint32_t)(now - ps2_last_edge_ccount) > 120000)
        ps2_bit = 0;
    ps2_last_edge_ccount = now;
...
static void ps2_key_down(uint8_t key)
{
    ...
    // Launcher actions use one ordinary key plus optional modifiers. Replacing
    // the old ordinary key prevents a lost break code from freezing controls.
