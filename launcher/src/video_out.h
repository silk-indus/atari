...
#define VIDEO_PIN 26
#define AUDIO_PIN 27 // PWM audio output
// IR disabled in standalone launcher (PS/2 only)
...
lldesc_t *_io_hold_desc = nullptr;
uint8_t *_io_hold_lines[11] = {0};
bool _io_hold_active = false;
...
{
    double audio_freq = ledcSetup(0, 100000, 7);
    printf("AUDIO LEDC actual=%f pin=%d\n", audio_freq, AUDIO_PIN);
...
void IRAM_ATTR blit_pal(uint8_t *src, uint16_t *dst)
{
    ...
    case EMU_ATARI:
        // Fast PAL Atari path.
        // Keep the same 4->5 horizontal stretch, but use nearest-neighbour
        // (A B B C D) instead of calculating three interpolated luma values
        // for every four source pixels inside the video ISR.
        left = 24;
        ...
        for (int i = left; i < right; i += 4)
        {
            ...
            // B again (nearest-neighbour stretch)
            dst[4 ^ 1] = P0;
            dst[5 ^ 1] = P1;
            ...
            // Continue packed output for the second 4-pixel group.
            color = p[c0];
            dst[10 ^ 1] = P2;
            dst[11 ^ 1] = P3;
...
{
    line += _burst_start;
    int16_t *b = ((_line_counter - 1) & 1) ? _burst0 : _burst1;
...
// Autonomous PAL frame for SD/SPIFFS operations. DMA repeats this complete
// frame without video ISR updates, so flash/SD latency cannot move the image.
bool video_io_hold_begin(const uint8_t *text_rows, int text_y)
{
    if (!_pal_ || !text_rows) return false;
    if (_io_hold_active) return true;

    if (!_io_hold_desc) {
        const int line_bytes = _line_width * sizeof(uint16_t);
        _io_hold_desc = (lldesc_t *)heap_caps_calloc(PAL_LINES, sizeof(lldesc_t), MALLOC_CAP_DMA);
        if (!_io_hold_desc) return false;

        for (int i = 0; i < 11; ++i) {
            _io_hold_lines[i] = (uint8_t *)heap_caps_calloc(1, line_bytes, MALLOC_CAP_DMA);
            if (!_io_hold_lines[i]) {
                for (int j = 0; j < 11; ++j) {
                    if (_io_hold_lines[j]) heap_caps_free(_io_hold_lines[j]);
                    _io_hold_lines[j] = nullptr;
                }
                heap_caps_free(_io_hold_desc);
                _io_hold_desc = nullptr;
                return false;
            }
        }

        // Shared ordinary blank, vertical-sync, and black active scanlines.
        blanking((uint16_t *)_io_hold_lines[0], false);
        blanking((uint16_t *)_io_hold_lines[1], true);
        blanking((uint16_t *)_io_hold_lines[2], false);
        uint8_t black_pixels[384] = {0};
        blit(black_pixels, (uint16_t *)_io_hold_lines[2] + _active_start);

        for (int row = 0; row < 8; ++row) {
            blanking((uint16_t *)_io_hold_lines[3 + row], false);
            blit((uint8_t *)text_rows + row * 384,
                 (uint16_t *)_io_hold_lines[3 + row] + _active_start);
        }

        for (int i = 0; i < PAL_LINES; ++i) {
            int buffer = 0;
            if (i >= 304 && i < 307)
                buffer = 1;
            else if (i >= 32 && i < 272) {
                int active_y = i - 32;
                buffer = (active_y >= text_y && active_y < text_y + 8)
                             ? 3 + active_y - text_y
                             : 2;
            }
            _io_hold_desc[i].buf = _io_hold_lines[buffer];
            _io_hold_desc[i].owner = 1;
            // EOF on every descriptor lets the task observe the static frame
            // position even while the CPU interrupt itself is disabled.
            _io_hold_desc[i].eof = 1;
            _io_hold_desc[i].length = line_bytes;
            _io_hold_desc[i].size = line_bytes;
            _io_hold_desc[i].empty = (uint32_t)&_io_hold_desc[(i + 1) % PAL_LINES];
        }
    }

    // With two dynamic DMA buffers, counter value 2 is observed immediately
    // after physical line 311: buffers 0 and 1 already contain lines 0 and 1.
    // Switch there so the autonomous ring starts at the exact PAL boundary.
    bool aligned = false;
    ...
    I2S0.int_clr.val = 0xFFFFFFFF;
    I2S0.out_link.addr = (uint32_t)_io_hold_desc;
    I2S0.out_link.start = 1;
    _io_hold_active = true;
    return true;
}

void video_io_hold_end()
{
    if (!_io_hold_active) return;

    // Prepare the first two dynamic scanlines while autonomous DMA is still
    // running. After they are played, the ISR continues by generating line 2.
    _line_counter = 1;
    blanking((uint16_t *)_dma_desc[0].buf, false);
    _line_counter = 2;
    blanking((uint16_t *)_dma_desc[1].buf, false);

    // Likewise, leave the autonomous ring only after its final scanline.
    bool stopped = false;
    for (int guard = 0; guard < 2000; ++guard) {
        if ((lldesc_t *)I2S0.out_eof_des_addr == &_io_hold_desc[PAL_LINES - 1]) {
            I2S0.out_link.stop = 1;
            stopped = true;
            break;
        }
...
void video_sync()
{
    ...
    if (_pal_)
    {
        // PAL active video occupies physical lines 32..271. Poll for the
        // following blanking interval instead of delaying by whole RTOS ticks,
        // which can overshoot into the next frame.
        for (int guard = 0; guard < 1000; ++guard)
        {
            int line = _line_counter;
            if (line >= 272 && line < 300)
                return;
...
extern "C" void IRAM_ATTR video_isr(volatile void *vbuf)
{
    ...
    if (_pal_)
    {
        // PAL: stable simple vertical sync
        if (i < 32)
        {
            blanking(buf, false);
        }
